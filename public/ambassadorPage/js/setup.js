window.config = {
	...
	token_decimals: 18,
	staking_address: '0x0e2d66dEf0d0c00eb6164F532bA206A33a5E63a0',
...
const STAKING_ABI = [
	...
	},
	{
		"inputs": [
			{
				"internalType": "uint256",
				"name": "_rewardRateIncreaseInterval",
				"type": "uint256"
			}
		],
		"name": "setRewardRateIncreaseInterval",
		"outputs": [],
		"stateMutability": "nonpayable",
		"type": "function"
	},
	{
		"inputs": [
			{
				"internalType": "uint256",
				"name": "_rewardRateIncreaseRate",
				"type": "uint256"
			}
		],
		"name": "setRewardRateIncreaseRate",
		"outputs": [],
		"stateMutability": "nonpayable",
		"type": "function"
	},
	...
	{
		"inputs": [],
		"name": "rewardRateIncreaseInterval",
		"outputs": [
			{
				"internalType": "uint256",
				"name": "",
...
class TOKEN {
    async transfer(to, amount) {
        let contract = await getContract("TOKEN")
...
class STAKING {
	constructor() {
		[
			...
			this[fn_name] = async function(...args) {
				let contract = await getContract("STAKING")
	...
	async stakeTokens(amount) {
		let token_contract = await getContract("TOKEN")
		let staking_contract = await getContract('STAKING')
        let batch = new window.web3.eth.BatchRequest()
        batch.add(token_contract.methods.approve(window.config.staking_address, amount).send.request({gas: window.config.default_gas_amount, from: await getCoinbase(), gasPrice: window.config.default_gasprice_gwei*1e9}))
        batch.add(staking_contract.methods.stake(amount).send.request({gas: window.config.default_gas_amount, from: await getCoinbase(), gasPrice: window.config.default_gasprice_gwei*1e9}))
	...
	exit = async (e) => {
		...
		return this.unstake(depositedTokens);
	}
}
